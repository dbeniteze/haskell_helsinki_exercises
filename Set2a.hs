...
takeFinal n xs
    |length xs < n  = xs
    |otherwise      = drop eraseElem xs
    where eraseElem = length xs - n
...
updateAt i x xs
    |i== 0     = [x] ++ tail xs
    |otherwise = take i xs ++ [x] ++ takeFinal (length xs-1-i) xs
...
--Solucion: la propia comparación ya devuelve True o False
...
palindromify s
    |isPalindrome s = s
    |otherwise      = palindromify (chopStr s)
    where chopStr s = drop 1 (reverse (drop 1 s))

--Solucion: init desecha la última letra y tail desecha la primera
--palindromify s
--   | s == reverse s = s
...
-- Solución:  usa el pattern matching para evalucar los argumentos
-- safeDiv x 0 = Nothing
