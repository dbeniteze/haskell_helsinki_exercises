...
allEqual [] = True
allEqual (xs)
    | map (const (head$nub xs)) [1..length xs]== xs = True
    | otherwise                                     = False 

-- Solución: uso de la función all, la desconocía.
-- allEqual [] = True
...
longest :: Ord a => [[a]] -> [a] 
longest xs 
    | length maxElem == 1 = head (reverse (sortByLength xs))
    | otherwise           = head (sortbyFirst maxElem)
        where maxlen  = maximum (map (length) xs)
              maxElem = [x | x <- xs, length x == maxlen]


sortByLength   = sortBy (comparing length)
sortbyFirst xs = sortBy (comparing head) xs   

-- Solución:
-- en la versión con sortBy 
--longest = last . sortBy (comparing length) . reverse . sortBy (comparing head)

-- en la versión sin sorts:
{-
...
incrementKey a [] = [] 
incrementKey a (x:xs)
    | fst x == a =  (fst x, (snd x)+1):incrementKey a xs
    | otherwise = x:incrementKey a xs

-- Solución: usa map para incrementar en 1 los values de la lista de pares.
...
winner scores player1 player2
    | getValue player1 >=  getValue player2 = player1
    | otherwise                             = player2
    where getValue x = Map.findWithDefault 0 x scores 
...
freqs xs = Map.fromList (pairKV xs ks)
       where ks = nub xs

pairKV [] ks = []
pairKV xs [] = []      
pairKV xs (k:ks) = (k, length (filter (==k) xs)) : pairKV xs ks 
...
transfer from to amount bank
        | checkAccounts from to bank   = checkAmount from to amount bank
        | otherwise                    = bank
      
checkAccounts from to bank = Map.member from bank && Map.member to bank 

checkAmount from to amount bank
        | amount < 0                             = bank
        | (sumAmount >= 0)&&(extractAmount >= 0) = doTransfer
        | otherwise                              = bank
        where sumAmount        = getValue to   + amount
              extractAmount    = getValue from - amount
              getJust (Just x) = x
              getValue x       = getJust (Map.lookup x bank)
              modTo            = Map.insert to sumAmount 
              modFrom          = Map.insert from extractAmount 
              doTransfer       = modTo (modFrom bank)
-- Solución: 
-- se usan 2 funciones como posibilidades de salida en case of y luego una
-- condición. Solución esta vez mucho mejor que la dada.
...
maxIndex array = (\(i,e) -> i) $ head .filter (\(i,e) -> e==largest) $ (assocs array)
    where largest = maximum $ map (array!) (indices array)
          
--sortByElemLength = sortBy (comparing length)
