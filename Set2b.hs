...
myGcd a b 
    | a == 0 || b == 0 = (a + b)
    | otherwise        = myGcd (min a b) restaEu
        where restaEu  = (max a b) - (min a b)
------------------------------------------------------------------------------
...
countdown i = "Ready! " ++ cuenta i ++ " Liftoff!"
    where cuenta 0 = ""
          cuenta i = show i ++ "... " ++ cuenta (i-1)
...
smallestDivisor n = smallestDivisor' n (n-1)

smallestDivisor' n t
    | mod n t == 0 && t >= 1 = div n t
