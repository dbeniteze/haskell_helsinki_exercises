...
totalPrice (MkShoppingEntry item price count) = fromIntegral(count)*price
...
fred :: Person
fred = PersonUndefined {age=90, name="Fred"}
...
setName name p = PersonUndefined {age=age p, name=name}

-- setAge does likewise for age
setAge :: Int -> Person -> Person
setAge age p = PersonUndefined {age=age, name=name p}
...
data Position = PositionUndefined {x::Int, y::Int}

-- origin is a Position value with x and y set to 0
origin :: Position
origin = PositionUndefined {x = 0, y = 0}  
...
up pos = PositionUndefined {x=(getX pos), y= 1 + (getY pos)}

-- right increases the x value of a position by one
right :: Position -> Position
right pos = PositionUndefined {x=(1 + getX pos), y=(getY pos)}

-- Solución: pattern matching en el argumento
-- up increases the y value of a position by one
...
data UpDown = Up {counter::Int} | Down {counter::Int} 
...
toNat z   
  | z < 0     = Nothing 
  | otherwise = Just (nat z) 
  where nat 0 = Zero
        nat z = (PlusOne (nat (z-1)))
...
fromBin :: Bin -> Int
fromBin bin = digitConv (0) (reverse (prettyPrint bin))

digitConv z [] = 0
digitConv z (x:xs)
  | x == '0' = 0 + digitConv (z+1) xs
  | x == '1' = 2^z + digitConv (z+1) xs

toBin :: Int -> Bin
toBin 0 = O End
toBin z = inc (toBin (z-1))