...
firstHalf xs = take middle xs
    where middle = if isOdd then (div (len) 2) + 1 else (div (len) 2)
          isOdd = odd (len)
          len   = length xs
...
capitalize xs = unwords (map capitalizeFirts (words xs))
capitalizeFirts (x:xs) = toUpper (x) : xs
------------------------------------------------------------------------------
...
joinToLength a b = [s++v | s <- b, v <- b, a == length (s++v)]


-- joinToLength a b = filter len (cadena b)
--    where cadena [] = []
--          cadena (x:xs) = map (x++) b ++ cadena xs
--          len x = length x == a
...
interpreter [] = []
-- interpreter = todo
interpreter (xs) = interpreter' 0 0 xs

interpreter' a b [] = []
interpreter' a b (x:xs)
        | x == "up"     = interpreter' a (b+1) xs
        | x == "down"   = interpreter' a (b-1) xs
        | x == "right"  = interpreter' (a+1) b xs
        | x == "left"   = interpreter' (a-1) b xs
        | x == "printX" = show a:interpreter' a b xs
        | x == "printY" = show b:interpreter' a b xs
        | otherwise     = interpreter' a b xs
